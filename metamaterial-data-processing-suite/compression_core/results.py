...
def build_compression_result_row(
    *,
    sample_name: str,
    E: float,
    ...
    return [
        display_name,
        E / 1e9 if _is_present(E) else np.nan,
        yield_strength / 1e6 if _is_present(yield_strength) else np.nan,
        ultimate_strength / 1e6 if _is_present(ultimate_strength) else np.nan,
...
def write_compression_results(output_path: Path, rows):
...
def save_results_to_xlsx(
    ...
    *,
    ...
    row = build_compression_result_row(
    ...
    for cell in ws.iter_rows(min_row=2, max_col=1):
        if canonical_sample_name(cell[0].value) == canonical_input:
            found_row = cell[0].row
            break
...
def check_valid_column(
    ...
    for row, E in zip(group_rows, group_E):
        ...
        if band is None or is_within_subgroup_band(E, band):
