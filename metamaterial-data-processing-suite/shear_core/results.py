...
from .decisions import build_shear_subgroup_band
...
def write_shear_results(output_path: Path, rows):
...
def save_results_to_xlsx(
    ...
    *,
    ...
    row_values = build_shear_result_row(
...
def check_valid_column(
    ...
    for row in range(2, ws.max_row + 1):
        ...
        if get_subgroup(sample_name) == group_name:
            ...
            value = ws.cell(row=row, column=E_idx).value
            try:
                group_E.append(float(value))
            except (TypeError, ValueError):
                group_E.append(np.nan)

    band = build_shear_subgroup_band(group_E)
